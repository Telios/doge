...
class MujocoObject:
    ...
    def apply_force(self, force, torque=np.array([0.0, 0.0, 0.0]), point_on_body=np.array([0.0, 0.0, 0.0])):
        mujoco.mj_applyFT(self.model, self.data, force, torque, point_on_body, self.bodyid, self.data.qfrc_applied)
    ...
    def set_rotation(self, euler):
        quat = utils.euler_to_quat(euler)
    ...
    def reset_position_random(self, all=False):
